...
public abstract class mt_mainbase extends AppCompatActivity implements EventReceiver {
    private static final String TAG = "OpenVPNClientBase";
    private OpenVPNService mBoundService = null;
    private ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            mt_mainbase.this.mBoundService = ((LocalBinder) service).getService();
            Log.d(mt_mainbase.TAG, "CLIBASE: onServiceConnected: " + mt_mainbase.this.mBoundService.toString());
            mt_mainbase.this.mBoundService.client_attach(mt_mainbase.this);
            mt_mainbase.this.post_bind();
        }

        public void onServiceDisconnected(ComponentName className) {
            Log.d(mt_mainbase.TAG, "CLIBASE: onServiceDisconnected");
            mt_mainbase.this.mBoundService = null;
        }
    };
    ...
    protected void submitDeleteProfileIntentWithConfirm(final String profile_name) {
        OnClickListener dialogClickListener = new OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case -1:
                        mt_mainbase.this.submitDeleteProfileIntent(profile_name);
                        return;
                    default:
                        return;
                }
    ...
    protected void resolveExternalPkiAlias(Profile prof, final EpkiPost next_action) {
        ...
        final mt_mainbase topthis = this;
        ...
        HttpsClient.run_task(Task, new Task() {
				public void run() {
					try {
						...
						Log.e(mt_mainbase.TAG, "import_profile", e2);
					...
					handler.post(new Runnable() {
							public void run() {
								if (!cancel.is_canceled()) {
									if (str != null) {
										mt_mainbase.this.submitImportProfileIntent(profile_content, str, true);
									} else {
										mt_mainbase.this.submitImportProfileIntent(profile_content, "client.ovpn", true);
									}
