...
public class mt_main extends mt_mainbase implements OnRequestPermissionsResultCallback, OnClickListener, OnTouchListener, OnItemSelectedListener, OnEditorActionListener {
    ...
    private Runnable stats_timer_task = new Runnable() {
        public void run() {
            mt_main.this.show_stats();
            mt_main.this.schedule_stats();
        }
    };
    ...
    private Runnable ui_reset_timer_task = new Runnable() {
        public void run() {
            if (!mt_main.this.is_active()) {
                mt_main.this.ui_setup(mt_main.RETAIN_AUTH, mt_main.UIF_RESET, null);
            }
        }
    };
        ...
        switch (ev.res_id) {
            ...
            case R.string.warn_msg /*2131034390*/:
                ...
                ok_dialog(resString(R.string.warning_title), ev.info, new Runnable() {
						public void run() {
							if (!(mt_main.this.delayed_finish_on_connect == FinishOnConnect.PENDING || mt_main.this.delayed_finish_on_connect == FinishOnConnect.DISABLED)) {
								self.finish();
							}
							mt_main.this.delayed_finish_on_connect = FinishOnConnect.DISABLED;
						}
					});
                break;
        }
        ...
        if (ev.res_id == R.string.connected && this.finish_on_connect != FinishOnConnect.DISABLED) {
            if (this.prefs.get_boolean("autostart_finish_on_connect", RETAIN_AUTH)) {
                ...
                new Handler().postDelayed(new Runnable() {
						public void run() {
							if (mt_main.this.finish_on_connect != FinishOnConnect.DISABLED) {
								self.finish();
							}
						}
					}, 1000);
    ...
    private void launch_create_profile_shortcut_dialog(final String prof_name) {
        ...
        DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case -1:
                        mt_main.this.createConnectShortcut(prof_name, name_field.getText().toString());
                        return;
                    default:
                        return;
                }
    ...
    private void launch_rename_profile_dialog(final String orig_prof_name) {
        ...
        DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case -1:
                        mt_main.this.submitRenameProfileIntent(orig_prof_name, name_field.getText().toString());
                        return;
                    default:
                        return;
                }
                ...
                switch (which) {
                    case -1:
                        if (proxy_list != null) {
                            ...
                            proxy_list.save();
                            mt_main.this.gen_ui_reset_event(mt_main.RETAIN_AUTH);
                            return;
                        }
                        return;
                    default:
    ...
    private void forget_creds_with_confirm() {
        ...
        DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case -1:
                        mt_main.this.pwds.regenerate(true);
                        ProfileList proflist = mt_main.this.profile_list();
                        if (proflist != null) {
                            proflist.forget_certs();
                        }
                        TrustMan.forget_certs(context);
                        OpenVPNImportProfile.forget_server_history(mt_main.this.prefs);
                        ProxyList proxy_list = mt_main.this.get_proxy_list();
                        if (proxy_list != null) {
                            proxy_list.forget_creds();
                            proxy_list.save();
                        }
                        mt_main.this.ui_setup(mt_main.this.is_active(), mt_main.UIF_RESET, null);
                        return;
                    default:
                        return;
                }
